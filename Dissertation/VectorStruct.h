...
struct dim6 {
	...
	__host__ __device__ dim6() : Bvec(0.0), Acen(0.0), Aipos(0.0), Aisub(0.0), Ajpos(0.0), Ajsub(0.0) {}
...
struct SparseIndex {
	vec2 Acen, Aipos, Aisub, Ajpos, Ajsub;
	__host__ __device__ SparseIndex() : Acen(vec2()), Aipos(vec2()), Aisub(vec2()), Ajpos(vec2()), Ajsub(vec2()) {}
	__host__ __device__ SparseIndex(vec2 Acen, vec2 Aipos, vec2 Aisub, vec2 Ajpos, vec2 Ajsub)
		: Acen(Acen), Aipos(Aipos), Aisub(Aisub), Ajpos(Ajpos), Ajsub(Ajsub) {}
};
